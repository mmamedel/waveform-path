...
export function linearPathFromAudioData(
  normalizeData: number[][],
  options: LinearPathOptions
) {
  const {
    samples = normalizeData.length,
    height = 100,
    width = 800,
    top = 0,
    left = 0,
...
export function getFilterDataWithOffset(
  framesData: Float32Array[],
  samples: number,
  start = 0,
  end = framesData.length
) {
  const filteredData = [] as number[][];
  for (let f = start; f < end; f++) {
    const blockSize = Math.floor(framesData[f].length / samples); // the number of samples in each subdivision
...
export function getMaxMultipler(framesData: Float32Array[]) {
  const multipliers = [] as number[];
  const filteredDataLength = framesData.length;
  for (let i = 0; i < filteredDataLength; i++) {
    const multiplier = getMax(framesData[i]);
    multipliers.push(multiplier);
  }
  return getMax(multipliers);
}

export function getNormalizeDataFromMaxMultipler(
  framesData: Float32Array[],
  maxMultiplier: number
) {
  const filteredDataLength = framesData.length;
  const normalizeData = [] as Float32Array[];
  for (let i = 0; i < filteredDataLength; i++) {
    const normalizeDataBlock = framesData[i].map((n) => n * maxMultiplier);
    normalizeData.push(normalizeDataBlock);
  }
  return normalizeData;
}
